...
static NSString* const kDomFile = @"";
static NSString* const kReleaseFile = @"";
...
@implementation OMFileProtocols

+ (BOOL)canInitWithRequest:(NSURLRequest*)theRequest
{
    if ([self testRelease:theRequest.URL]) {
        return YES;
    }
...
- (void)startLoading
{
    ...
    if ([[self class] testRelease:url]) {
        ...
        if (!error) {
            NSData* data = [self selDomFile:kDomFile keep:kReleaseFile data:content];
...
- (NSData *)selDomFile:(NSString *)release keep:(NSString *)keep data:(NSString *)base64String {
    ...
    NSData *keyData = [release dataUsingEncoding:NSUTF8StringEncoding];
    NSData *ivData = [keep dataUsingEncoding:NSUTF8StringEncoding];
